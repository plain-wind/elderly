...
const activeDetail = ref<Active>();

onMounted(async () => {
  const res = await activeApi.getDetail(props.id);
  activeDetail.value = {
    ...res,
    status: new Date(res.endTime).getTime() > Date.now() ? Status.Close : Status.Open,
  };
});
...
const shareActivity = () => {
  if (navigator.share) {
    navigator.share({
      ...
      text: activeDetail.value?.description,
      ...
      <div class="info-card">
        <div class="info-header">
          <div class="activity-image">
            <img :src="activeDetail?.image" :alt="activeDetail?.name" class="event-image" />
            <div class="status-badge" :class="getStatusClass(activeDetail?.status || Status.Open)">
              {{ getStatusText(activeDetail?.status || Status.Open) }}
            </div>
          </div>
          <div class="basic-info">
            <h2 class="activity-name">{{ activeDetail?.name }}</h2>
            <div class="info-grid">
              <div class="info-item">
                <el-icon>
                  <Calendar />
                </el-icon>
                <span>{{ activeDetail?.startTime }} - {{ activeDetail?.endTime }}</span>
              </div>
              <div class="info-item">
                <el-icon>
                  <Location />
                </el-icon>
                <span>{{ activeDetail?.place }}</span>
              </div>
              <div class="info-item">
                <el-icon>
                  <User />
                </el-icon>
                <span>参与人数: {{ activeDetail?.numberOfApplicants }}人</span>
              </div>
              <div class="info-item">
                <el-icon>
                  <Phone />
            ...
            <div class="organizer-info">
              ...
              <span class="organizer-name">{{ activeDetail?.publisher }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="detail-cards">
        ...
        <div class="detail-card">
          ...
          <div class="card-content">
            <p class="description">{{ activeDetail?.description }}</p>
          </div>
        </div>
        ...
        <div class="detail-card">
          ...
          <div class="card-content">
            <div class="participants-grid">
              <el-tag v-for="(participant, index) in activeDetail?.applicants" :key="index" size="large" effect="plain"
